...
export default function POS() {
  ...
  // For QRIS: no change needed — exact amount
    ...
    try {
      ...
      const { error: itemsErr } = await supabase.from('order_items').insert(
        cart.map(i => ({
          ...
          product_price: i.price, quantity: i.qty, subtotal: i.price * i.qty,
        }))
      )
      ...
      await supabase.from('payments').insert({
        order_id: order.id, store_id: profile.store_id,
        amount: total, method: payMethod, status: 'success',
      })
  ...
  // ── Cart panel (shared between mobile drawer and desktop sidebar) ──
  const CartPanel = () => (
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 border-b border-gray-800 flex justify-between items-center flex-shrink-0">
        ...
        <button onClick={() => setCart([])} className="text-xs text-gray-400 hover:text-white">{t.clearCart}</button>
      </div>
      ...
      <div className="px-4 py-4 border-t border-gray-800 space-y-3 flex-shrink-0">
        <div className="flex justify-between font-bold text-lg">
          <span>{t.total}</span><span>{formatRp(total)}</span>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            {[
              ...
              <button key={val} onClick={() => { setPayMethod(val); setCash('') }}
        ...
        {orderError && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 rounded-lg p-2 text-xs">{orderError}</div>
        )}
    ...
    <div className="h-screen bg-gray-950 text-white flex flex-col overflow-hidden">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2 flex-shrink-0">
          {plan === 'free' && (
            <span className={`text-xs hidden sm:inline ${todayCount >= DAY_LIMIT ? 'text-red-400' : todayCount >= 80 ? 'text-amber-400' : 'text-gray-500'}`}>
            ...
            {filtered.map(p => (
              <button key={p.id} onClick={() => addToCart(p)}
                ...
                <div className="p-2">
                  ...
                  <div className={`text-xs mt-0.5 ${p.stock === 0 ? 'text-red-400' : p.stock <= 5 ? 'text-amber-400' : 'text-gray-500'}`}>
                    {p.stock === 0 ? t.outOfStock : p.stock <= 5 ? `${t.lowStock} ${p.stock} ${t.left}` : `${p.stock} ${t.available}`}
                  </div>
                </div>
              </button>
            ))}
