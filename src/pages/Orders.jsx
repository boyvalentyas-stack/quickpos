            ...
            {order.payment_method === 'cash' && (
              <>
                <div className="flex justify-between text-sm text-gray-300">
                  ...
                  <span>{formatRp(order.cash_received || 0)}</span>
                </div>
                <div className="flex justify-between text-sm font-bold text-green-400">
                  <span>Change</span>
                  <span>{formatRp(order.change_amount || 0)}</span>
                </div>
              </>
            )}
      ...
      <div className="max-w-5xl mx-auto px-4 py-5">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
            <div className="text-xs text-gray-400 uppercase tracking-widest mb-1">Transactions</div>
            <div className="font-bold text-xl">{completedOrders.length}</div>
          </div>
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
            <div className="text-xs text-gray-400 uppercase tracking-widest mb-1">Revenue</div>
            <div className="font-bold text-xl text-violet-400 truncate">{formatRp(totalRevenue)}</div>
          </div>
        </div>
            ...
            <thead>
              <tr className="border-b border-gray-800">
                {['Receipt No.', 'Date & Time', 'Items', 'Cashier', 'Total', 'Payment', 'Status'].map(h => (
                  <th key={h}
                    className="text-left px-4 py-3 text-xs text-gray-400 uppercase tracking-widest font-normal">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filtered.map(o => (
                <tr key={o.id}
                  ...
                  <td className="px-4 py-3">
                    <span className="font-mono font-bold text-sm text-violet-400 hover:text-violet-300">
                      {o.order_number}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-300 max-w-[120px] truncate">
                    {o.users?.full_name || '—'}
                  </td>
